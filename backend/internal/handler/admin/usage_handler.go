...
func (h *UsageHandler) Stats(c *gin.Context) {
	...
	if startDateStr != "" && endDateStr != "" {
		var err error
		startTime, _, err = timezone.ParseDateOrMinute(startDateStr, userTZ)
		if err != nil {
			response.BadRequest(c, "Invalid start_date format, use YYYY-MM-DD or YYYY-MM-DDTHH:mm")
			return
		}
		var endHasTime bool
		endTime, endHasTime, err = timezone.ParseDateOrMinute(endDateStr, userTZ)
		if err != nil {
			response.BadRequest(c, "Invalid end_date format, use YYYY-MM-DD or YYYY-MM-DDTHH:mm")
			return
		}
		// 与 SQL 条件 created_at < end 对齐：纯日期用次日 00:00 作为上边界（DST-safe），
		// 带时间则精确到所给时刻。
		if !endHasTime {
...
func (h *UsageHandler) CreateCleanupTask(c *gin.Context) {
	...
	startTime, _, err := timezone.ParseDateOrMinute(req.StartDate, req.Timezone)
	if err != nil {
		response.BadRequest(c, "Invalid start_date format, use YYYY-MM-DD or YYYY-MM-DDTHH:mm")
		return
	}
	endTime, endHasTime, err := timezone.ParseDateOrMinute(req.EndDate, req.Timezone)
	if err != nil {
		response.BadRequest(c, "Invalid end_date format, use YYYY-MM-DD or YYYY-MM-DDTHH:mm")
		return
	}
	// 过滤条件为 created_at <= end：纯日期含当天最后一刻，带时间精确到所给时刻。
	if !endHasTime {
