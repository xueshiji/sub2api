export default {
    accounts: {
      ...
      perfStatsHint: '账号最近 30 分钟的平均表现（跨模型聚合）：TTFT 为流式请求首字延迟；decode 速度为输出 token 的加权吞吐，仅统计流式请求（按时长扣除首字延迟计算），非流式请求不计入。得分为相对窗口内全站最优账号的加权相对性能（TTFT 与 decode 各占一半），样本不足 3 的账号不计分，可按得分在服务端排序。开启 prefer_best_performance 时，调度器在同优先级候选内按当前请求映射后的上游模型维度，以「性能分 × 负载折扣 × 慢惩罚」联合评分优选（perf_load_penalty_slope > 0 时；否则仅在同负载率候选内按性能优选）。「慢惩罚」标记表示该账号连续出现慢请求，调度评分临时乘惩罚系数，到期自动解除。',
      ollamaCloud: {
