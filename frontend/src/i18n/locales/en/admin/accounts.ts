export default {
    accounts: {
      ...
      perfStatsHint: 'Average performance of the account over the last 30 minutes (aggregated across models): TTFT is the first-token latency of streaming requests; decode speed is the weighted throughput of output tokens for streaming requests only (computed over duration excluding first-token latency); non-streaming requests are excluded. The score is the weighted relative performance against the best account in the window (TTFT and decode weighted equally); accounts with fewer than 3 samples get no score. Sorting by score is done server-side. When prefer_best_performance is enabled, the scheduler ranks same-priority candidates per request-mapped upstream model by the joint score "performance × load discount × slow penalty" (when perf_load_penalty_slope > 0; otherwise by performance among equal-load candidates). The "Slow penalty" badge means the account recently hit consecutive slow requests, so its scheduling score is temporarily reduced until the penalty expires.',
      ollamaCloud: {
